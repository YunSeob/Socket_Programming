...
int main(int argc, char *argv[]){
    ...
    snprintf(send_msg, sizeof(send_msg), "SETUP %s/trackID=m RTSP/1.0\r\n"
                                         "CSeq: %d\r\n"
                                         "User-Agent: ys\r\n"
    ...
    // if(connect(client_sock, (struct sockaddr*)&client_addr, sizeof(client_addr)) == SOCKET_ERROR){
    ...
    // FILE *fp = fopen("udp.txt", "w");
    while(count < 50){
        bytes = recv(camera_sock, buffer, BUFFER_SIZE, 0);
